...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "တခြားလုပ်ဆောင်ရမယ့် အရာတွေကို ယုံကြည်စွာ ကူညီဖြေရှင်းပေးဖို့ အသင့်ပါဗျ။\n"
        "ဒီနေ့အတွက် သင့်ရဲ့ ကံကြမ္မာကို ဖတ်ကြည့်ချင်ရင် /daily ကိုနှိပ်ပါ။\n\n"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        # update.message ရှိမရှိ စစ်ပါ
        if not update.message:
            # အကယ်၍ update.message မရှိရင်၊ callback_query ကနေ ပြန်ဖြေမယ်
        ...
        # အမှားတစ်ခုခုဖြစ်ရင် သတိပေးစာပြန်ပေးမယ်
        logger.error(f"Help command error: {e}")
        if update.message:
            await update.message.reply_text(
                "⚠️ Help command မှာ အမှားတစ်ခုဖြစ်သွားပါတယ်။\n"
                f"Error: {str(e)[:100]}"
            )
